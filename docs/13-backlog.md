...
| US-081 | Wire trust path + ABI diff + storage diff + source diff into apps/web/lib/loadReport.ts (currently posiela null, takže verdict engine sees empty features and vault scenario reports SIREN instead of SAFE). Use existing primitives: verifyReportFromManifest (US-069), diffAbiRiskySelectors (US-026), diffStorageLayout (US-027), diffSourceFiles (US-075). | C | P0 | M | open | US-068, US-069, US-026, US-027, US-075 |
| US-082 | Public-read raw-address path: when /r/[name] receives a 0x-prefixed hex string, bypass tryResolveEns and call runPublicReadFallback (US-019) directly. Currently resolveEnsRecords rejects raw addresses, breaking Aave V3 Pool scenario. | C | P0 | S | open | US-019, US-068 |
...
| US-084 | Five-second performance fix: live /r/[name] currently exceeds 5s budget on dangerous + unverified scenarios (Playwright e2e ~9.5s and ~9.9s). Either prewarm cache for demo subnames + add ?mock=true fallback that bypasses live fetch, OR optimize parallel fetches (ENS + EIP-1967 + Sourcify in Promise.all). GATE-20 currently failing. | C | P0 | M | open | US-068, US-081, US-032, US-033 |
